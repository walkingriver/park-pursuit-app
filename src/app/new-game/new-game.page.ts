...
export class NewGamePage implements OnInit {
  ...
  async ngOnInit() {
    ...
    let unfoundClues = (await this.clueService.getClues()).filter(
      (c) => !c.isFound
    );
    this.parks = parks;
    this.clueCounts = parks.map((p) => {
      return {
        code: p.code,
        clueCount: unfoundClues.filter((c) => c.parkCode === p.code).length,
      };
    });
  }
  ...
  async newGame(park: Park) {
    ...
    newGame.clueList = clueList.slice(
      0,
      this.clues < clueList.length ? this.clues : clueList.length
    );
